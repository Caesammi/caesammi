 ...
 * params {Function} errorCallback 请求失败回调函数
 */
function ajaxGetLst(url, yesCallback, errorCallback) {
       getUserCode(function (userCode) {
           var str = "?"
           if (url.indexOf('?') > 0) {
               str = "&";
...
//                 var page = '';
//                 var newResult = JSON.parse(data).collection;
//                 for (var k in newResult) {
//                     if (k === 'page') {
//                         page = newResult.page;
//                     }
//                 }
                   yesCallback && yesCallback(data);
               },
               error: function (data) {
  ...
  function getUserCodeToPM() {
      if (GetQueryString("dev")) {
         return   {data:{userCode:"zhouxy210"}} ;
...
function getUserCode(callback) {
    if (callback=="dev") {
       ...
       $.ajax({
            url: 'http://iplant.spc.com/disc-web/getUserCode',
            type: 'get',
           success:function (user) {
              return (user.data.userCode);
           },
           error:function () {
               return ("zhouxy210");
           }
