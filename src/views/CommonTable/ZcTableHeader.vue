<template>
  <el-table-column
    :width="data.columnWidth ? data.columnWidth : ''"
  ...
  >
    ...
    >
      <div
        class="headerBox"
        :style="{color: data.titleColor, fontSize: '16px'}"
      >
        {{ data.name }} <span :style="{color: data.titleUnitColor}">{{ data.unit }}</span>
      </div>
    </template>
    <template
      v-if="!data.children"
    ...
    >
      ...
      >
        <p
          ...
          :style="{color: '#00d539',lineHeight:columnHeight ? columnHeight : ''}"
        >
          +{{ scope.row[data.value] }}
        </p>
        <p
          v-if="parseFloat(scope.row[data.value]) === 0"
          :style="{color: data.color,lineHeight:columnHeight ? columnHeight : ''}"
        >
          {{ Math.abs(scope.row[data.value]) }}
        </p>
        <p
          v-if="parseFloat(scope.row[data.value]) < 0"
          :style="{color: '#ff0000',lineHeight:columnHeight ? columnHeight : ''}"
        >
          {{ scope.row[data.value] }}
        </p>
      </div>
      ...
      >
        <p :style="{color: data.color,lineHeight:columnHeight}">
          {{ scope.row[data.value] ? scope.row[data.value] : '--' }}
        </p>
      </div>
      <!--      </el-tooltip>-->
      <slot
        v-if="data.slotType==='text'"
        name="tableSlot"
      />
    </template>
